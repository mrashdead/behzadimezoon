...
const saerchList = () => {
  ...
  // JSON load
  const searchJsonUrl = window.APP_CONFIG?.searchJsonUrl;

  if (searchJsonUrl) {
    $.getJSON(searchJsonUrl, function (data) {
      listItems = data.listItems;
    }).fail(function (jqxhr, textStatus, error) {
      console.error("Failed to load search JSON:", textStatus, error);
    });
  }
...
$(document).on(
  "mouseenter",
  ".is-week-mode td:not(.new):not(.old)",
  function () {
...
$(document).ready(function () {
  ...
  function updateInputAndStyles() {
    var startStr = rangeState.start
      ? new persianDate(rangeState.start).format("YYYY/MM/DD")
      : "";
    var endStr = rangeState.end
      ? new persianDate(rangeState.end).format("YYYY/MM/DD")
      : "";

    var $input = $(".range-input-target:focus").length
      ? $(".range-input-target:focus")
      : $(".range-input-target");
  ...
  function applyRangeStyles() {
    setTimeout(function () {
      ...
      $container
        .find("td")
        .removeClass(
          "range-start-custom range-end-custom range-between-custom",
        );

      // اعمال کلاس مبدأ
      if (rangeState.start) {
        $container
          .find('td[data-date="' + rangeState.start + '"]')
          .addClass("range-start-custom");
      }

      // اعمال کلاس مقصد و روزهای بین
      if (rangeState.end) {
        $container
          .find('td[data-date="' + rangeState.end + '"]')
          .addClass("range-end-custom");
  ...
  $(document).on(
    "click",
    ".pwt-btn-next, .pwt-btn-prev, .month-item, .year-item",
    function () {
      applyRangeStyles();
    },
  );
  ...
  function applyMultipleStyles() {
    setTimeout(function () {
      ...
      selectedDates.forEach(function (unix) {
        $container
          .find('td[data-date="' + unix + '"]')
          .addClass("selected-multiple");
      });
    }, 50);
  }
